...
struct TRTCPublishTarget {
    ...
    /// from the users whose streams are transcoded. You call {@link muteRemoteVideoStream} and {@link muteRemoteAudio} to unsubscribe from the transcoded stream. `Note:` The {@link muteAllRemoteAudio} and {@link muteAllRemoteVideoStreams} APIs do
    /// not apply to the data published by the robot. `Note:` Users whose streams are transcoded cannot subscribe to the transcoded stream.
