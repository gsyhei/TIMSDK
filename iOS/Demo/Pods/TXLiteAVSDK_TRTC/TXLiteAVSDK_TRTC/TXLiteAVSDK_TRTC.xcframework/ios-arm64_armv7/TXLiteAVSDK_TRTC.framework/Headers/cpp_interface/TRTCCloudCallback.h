...
class ITRTCCloudCallback {
   public:
     ...
     *  For more information on the error codes for room entry failure, see [Error Codes](https://intl.cloud.tencent.com/document/product/647/35124).
     * @note
     ...
     * You can then call {@link startRemoteSubStreamView} to subscribe to the remote user’s video. If the subscription is successful, you will receive the `onFirstVideoFrame(userid)` callback, which indicates that the first frame of the user is
     * rendered.
     * @param userId User ID of the remote user
 ...
 */
     ...
     * @param errCode Error code. `0`: recording succeeded; `-1`: recording failed; `-2`: recording was ended due to change of resolution or switch between the landscape and portrait mode；`-3`:The audio data or video data has not arrived so the
     * formal recording has not started.
     * @param storagePath Storage path of recording file
     */
